...
- **The timesheet AI summary NEVER computes a number.** `admin-timesheets-ai.facts.ts` copies the finished figures out of the DTO the dashboard already rendered - `capacityHoursForPeriod`, `measureAgainstTarget`, the aggregate pass - and the model only writes sentences about them. A model asked to derive utilisation will sometimes divide by five days for somebody contracted to three, and a plausible wrong number in the prose beside the right one in the tile discredits both. The system prompt says so as well, but what actually holds is that the arithmetic is already done.
- **A timesheet REPORT is a record, not a cache, and every difference from the summary follows from that.** No fingerprint and no staleness (a summary says how things are, a report says how they were); writing again inserts another row rather than replacing one; and `timesheet_report.facts` SNAPSHOTS the figures the prose quotes, because re-deriving them later from a re-synced read model gives different numbers and would make an old report unverifiable. `period_label` and `created_by_name` are snapshotted for the same reason `audit_logs` snapshots its actor. Its retention window is 365 days against the cache's 30, and unlike the cache what it sweeps CANNOT be regenerated.
- **A report may not be cheerier than the data, and truncation is where that breaks.** The prompt requires it to say the period is not invoiceable when `isBillable` is false or `blockingCount` is above zero. The finding list is capped at 30 and sorted BLOCKING FIRST so warnings can never crowd out the one thing that stops an invoice; jobs are sorted trouble-first (over estimate, then unestimated, then largest); and the true counts travel beside the capped lists so a report cannot report "30 findings" because 30 were sent. `admin-timesheets-report.facts.test.ts` asserts all of it.
- **Both report and summary prompts allow for an unfinished period.** A month still in progress is not a shortfall against capacity, and without that rule the model calls it a failing - which is what the first per-person summaries did.
...
- **Job and project names reaching the summariser are untrusted input**, on the same footing as the attachment filenames `sanitizeDocumentName` handles - staff type them in Jira. They travel inside `BEGIN FACTS`/`END FACTS` markers, the system prompt says content there is data and never instruction, and the reply is rendered through `AiChatMarkdown` (React elements, no `dangerouslySetInnerHTML`). Nothing the model returns is ever allowed to drive control flow.
- **A natural-language "custom view" must never become AI-generated SQL.** If that feature is built, the model returns a Zod-validated FILTER object the existing repositories already accept; repositories stay the only DB access. A model emitting SQL would break that rule and be an injection surface in the same breath.
