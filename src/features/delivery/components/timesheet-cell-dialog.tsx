...
// now - see TimesheetDayCell - so entering a week is typing and tabbing, and
// this dialog is reached deliberately: to write a note, to put a SECOND entry
// on a day that already has one, to pick between several, or to clear a day.
//
...
//
// THE ARGUMENT FOR THAT USED TO BE THE SCHEMA, AND THE SCHEMA CHANGED. It was
// written here that `UpdateTimeEntrySchema` treats an absent note as NULL, so
// any form opening blank would silently delete whatever was on the entry the
// moment somebody fixed an hour - and that note is what a client's invoice
// narrative is written from. That is no longer true and has not been for a
// while: `notes` is `patchText`, which is `.optional()`, so an absent note
// LEAVES the stored one and only `''` clears it. Same for `workDate`, which
// is what keeps a corrected hour from being re-stated at today's rate.
//
// So there are now two safe shapes and the app uses both, deliberately. A
// form that SHOWS the note sends it back explicitly - which is this one, and
// is why `TimesheetCellEntryDTO` carries `notes` beside `id` and `minutes`.
// A control that does not show the note omits the field - which is the grid
// cell, correcting a figure without ever touching what was written about it.
// Neither can lose a note. The old rule survives; only its reason moved.
//
...
//   one entry       the form opens on it, filled in. The ordinary case.
//   several         they are listed, each with its hours and its note, and
//                   picking one opens the form on it. Nothing is collapsed
//                   into a single figure that could not be typed back. This
//                   is also where the grid sends a split day, because a cell
//                   cannot correct one in place.
//
